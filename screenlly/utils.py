...
def take_screenshot(driver, file_path, top_left=(0, 0), bottom_right=None,
    ...
    def element_scroll_to(el, x, y):
        driver.execute_script("arguments[0].scrollTo(arguments[1], arguments[2]);", el, x, y)
    ...
    def get_screen_piece():
        ...
        x2 = bottom_right[0]
        y1 = 0
        current_y = element_get_current_y(scrollable_element) if scrollable_element else get_current_y()
    ...
    scrollable_element = driver.find_element_by_xpath(scrollable_element_xpath) if scrollable_element_xpath else None
    ...
    if scrollable_element:
        body_height = max([body_height, driver.execute_script('return arguments[0].scrollHeight', scrollable_element)])
    ...
    if scrollable_element:
        element_wait_position(scrollable_element, min(top_left[1], body_height - window_height))
    else:
    ...
    for y_position in y_positions:
        ...
        im = get_screen_piece()
        current_y = element_get_current_y(scrollable_element) if scrollable_element else get_current_y()
        screenshot.paste(im, (0, current_y + fixed_header_height if y_position > 0 else 0))
